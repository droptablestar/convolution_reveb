...
version 0.0.1, Dec 6, 2010

¥ unlike 0.0.2, doesn't crash.  the main problem is that you have to wait x->arraysize samples before you hear anything.  something also needs to be done about gain adjustment.  it currently goes a little over 100dB with voice.wav
...
static void convolve_tilde_analyze(convolve_tilde *x, t_symbol *s)
{
    ...
    x->nonoverlapped_output = (t_sample *)t_resizebytes(x->nonoverlapped_output, 0, x->overlap*x->arraysize*sizeof(t_sample));
    ...
    for(i=0; i<x->arraysize*x->overlap; i++)
...
{
    int i, j, n, window, window_half, ir_window, ir_window_half, overlap, overlap_chunk;
	float amp_scalar;
	...
	if(x->dsp_tick==x->buffer_limit)
	{
		...
		// pad the latest N samples with zeros out to the length of the IR
		for(; i<ir_window; i++)
			x->signal_buf_windowed[i] = 0.0;
		...
		// write the new block
		for(i=0; i<ir_window; i++)
			x->nonoverlapped_output[((overlap-1)*ir_window)+i] = x->signal_buf_windowed[i];
		...
		for(i=0; i<overlap; i++)
			for(j=0; j<overlap_chunk; j++)
				x->final_output[j] += x->nonoverlapped_output[(i*ir_window)+((overlap-i-1)*overlap_chunk)+j];
	};
...
{	
    ...
    t_freebytes(x->nonoverlapped_output, (x->arraysize*x->overlap)*sizeof(t_sample));
