...
¥ 0.0.2, shooting for a small buffered block size (N). version A requires that irlen==N in order to work.  Here, we make a longer nonoverlapped history so that the latest final_output can be summed based on that history.
...
static void convolve_tilde_analyze(convolve_tilde *x, t_symbol *s)
{
    ...
    i=6; // the IR has to be at least 64 samples long
    
    while(pow(2, i++) < x->arraysize)
    	;
    	
    newsize = pow(2, i-1);  // back up by one because the while overshoots

    x->buf_hist_size = newsize/x->overlap_chunk;
//	post("buf hist size: %i", x->buf_hist_size);
	
    x->ir_spec = (t_float *)t_resizebytes(x->ir_spec, oldsize, newsize*sizeof(t_float));
    x->signal_buf_windowed = (t_sample *)t_resizebytes(x->signal_buf_windowed, oldsize, newsize*sizeof(t_sample));
    x->nonoverlapped_output = (t_sample *)t_resizebytes(x->nonoverlapped_output, oldsize, x->buf_hist_size*newsize*sizeof(t_sample));

    // copy array internally for upcoming in-place FFT
    for(i=0; i<x->arraysize; i++)
    	x->ir_spec[i] = x->x_vec[i].w_float;

	// update x->arraysize to newsize, now that we don't need the original arraysize anymore
	x->arraysize = newsize;
	
	// pad out remainder (if any) with zeros
    for(; i<x->arraysize; i++)
		x->ir_spec[i] = 0.0;

	// init signal_buf_windowed
    for(i=0; i<x->arraysize; i++)
    	x->signal_buf_windowed[i] = 0.0;

    for(i=0; i<x->arraysize*x->buf_hist_size; i++)
...
static void *convolve_tilde_new(t_floatarg window, t_floatarg overlap)
{
	...
	if(window && overlap)
	{
		...
		if(isPow2)
		{
			...
			{
				x->window = MINWIN;
				error("convolve~: requested window size too small. minimum value of %i used instead", x->window);				
			}
		}
		else
		{
			x->window = 4096;
			error("convolve~: requested window size not a power of 2. default value of %i used instead", x->window);
		}

		isPow2 = (int)overlap && !( ((int)overlap-1) & (int)overlap );
		if(isPow2)
		{
			if(x->window/overlap >= 64)
				x->overlap = overlap;
			else
			{
				x->overlap = 2;
				error("convolve~: requested overlap value too great. default value of %i used instead", x->overlap);
			}
		}
		else
		{
			x->overlap = 2;
			error("convolve~: requested overlap not a power of 2. default value of %i used instead", x->overlap);
		}
	}
	else if(window)
	{
		...
		{
			x->window = 4096;
			error("convolve~: window not a power of 2. default value of %i used instead", x->window);
		}
...
static void convolve_tilde_dsp(convolve_tilde *x, t_signal **sp)
{
	...
	if(!x->arraysize)
		error("convolve~: impulse response analysis not performed yet. output will be zero.");
};
